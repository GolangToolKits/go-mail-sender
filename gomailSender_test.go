...
func TestGoMailSender_SendMail(t *testing.T) {
	//ports 587 465 80
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			if got := m.SendMail(tt.args.mailer); got != tt.want {
				t.Errorf("GoMailSender.SendMail() = %v, want %v", got, tt.want)
			}
		})
	}
}

func TestGoMailSender_New(t *testing.T) {
	type fields struct {
		User     string
		Password string
		MailHost string
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			if got := m.New(); got == nil{
				t.Errorf("GoMailSender.New() = %v, want %v", got, tt.want)
			}
		})
	}
}
